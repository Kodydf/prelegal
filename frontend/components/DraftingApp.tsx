...
export default function DraftingApp() {
  ...
  useEffect(() => {
    ...
    fetchDocument(documentId)
      ...
      .catch((err) => {
        if (!cancelled) setLoadError(err instanceof ChatError ? err.message : GENERIC_ERROR);
      });
    return () => {
      cancelled = true;
    };
      ...
      <section className="w-full lg:sticky lg:top-10 lg:w-[420px] lg:shrink-0">
        <div className="flex items-start justify-between gap-4">
          ...
          <button
            type="button"
            onClick={startOver}
            className="shrink-0 rounded-md border border-navy px-3 py-1 text-sm font-medium text-navy transition-colors hover:bg-silver/30"
          >
            Start over
          </button>
        </div>
        ...
        {ready ? (
          ...
          <div role="alert" className="rounded-lg border border-navy border-l-4 border-l-gold p-6 text-sm text-black">
            ...
            <button
              ...
              className="mt-3 rounded-md border border-navy px-3 py-1 font-medium text-navy hover:bg-silver/30"
            >
              Retry
            </button>
          </div>
